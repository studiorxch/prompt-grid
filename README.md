...
Sections and cards use a dense Kanban-style layout. Drag sections from the muted `⠿` header handle and drag cards from their body. Section rename/movement/deletion lives in a keyboard-accessible `…` menu; card Enable/Disable and Delete actions use the native context menu, while card movement is drag-only. Double-click card text, or focus it and press Enter, to open multiline editing; use Ctrl/Cmd+Enter to commit or Escape to cancel.
