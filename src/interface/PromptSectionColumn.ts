...
export class PromptSectionColumn {
 ...
 constructor(private readonly actions:SectionActions,private readonly menus:FloatingMenuController){this.cards=new PromptLineCard(actions.line,menus);}
 render(parent:HTMLElement,section:PromptSection|null,lines:PromptLine[],index:number,collapsed=false):void {
  ...
  if(section){const handle=header.createSpan({text:"⠿",cls:"prompt-grid-section-handle",attr:{draggable:"true","aria-label":"Drag section"}});handle.addEventListener("dragstart",event=>{event.dataTransfer?.setData("text/plain",section.id);this.actions.sectionDragStart(section.id);});header.addEventListener("dragover",event=>event.preventDefault());header.addEventListener("drop",event=>{event.preventDefault();event.stopPropagation();void this.actions.sectionDrop(index);});}
  else header.createSpan({text:"⠿",cls:"prompt-grid-section-handle is-static",attr:{"aria-hidden":"true"}});
  const collapse=header.createEl("button",{text:collapsed?"›":"⌄",cls:"prompt-grid-collapse",attr:{"aria-label":`${collapsed?"Expand":"Collapse"} ${section?.label??"Preamble"}`,"aria-expanded":String(!collapsed)}});collapse.addEventListener("click",()=>this.actions.toggleCollapse(id));
  header.createEl("strong",{text:section?.label??"Preamble",cls:"prompt-grid-column-title"});header.createSpan({text:String(lines.length),cls:"prompt-grid-card-count",attr:{"aria-label":`${lines.length} cards`}});if(section)this.menu(header,section);
  if(collapsed)return;
