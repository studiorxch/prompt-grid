...
import {collapseAriaExpanded,columnPresentationClass} from "./composerPresentation";
...
export class PromptSectionColumn {
 ...
 render(parent:HTMLElement,section:PromptSection|null,lines:PromptLine[],index:number,collapsed=false):void {
  const id=section?.id??"preamble",column=parent.createDiv({cls:columnPresentationClass(section===null,collapsed)});column.dataset.containerId=id;
